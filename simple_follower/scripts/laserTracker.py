...
class laserTracker:
	def __init__(self):
		...
		self.scanSubscriber = rospy.Subscriber('/summit_xl/front_laser/scan', LaserScan, self.registerScan)
	...
	def registerScan(self, scan_data):
		...
		if(not(self.lastScan is None)):
			...
			for i in sortedIndices:
				...
				with np.errstate(invalid='ignore'):
					...
					rospy.logwarn("Das hier")
					rospy.logwarn(np.any(abs(window-tempMinDistance)<=self.deltaDist))
		...
		if(minDistance > scan_data.range_max):
			...
			# rospy.logwarn(minDistance)
			# rospy.logwarn(scan_data.range_max)			
